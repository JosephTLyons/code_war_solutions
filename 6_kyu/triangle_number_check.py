...
# A triangle number is a number where n objects form an equilateral triangle (it's a bit hard to
# explain). For example, 6 is a triangle number because you can arrange 6 objects into an
...
# Check if a given input is a valid triangle number. Return true if it is, false if it is not (note
# that any non-integers, including non-number types, are not triangle numbers).
