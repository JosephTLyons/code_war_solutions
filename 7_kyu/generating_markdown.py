# Your friend has recently started using Codewars to learn more advanced coding. They have just
# created their first kata, and they want to write a proper description for it, using codeblocks,
# images and hyperlinks.

# However, they are struggling to understand how to use Markdown formatting properly, so they decide
# to ask for your help, by having you write a program that will generate some of the syntaxes for
# you.
...
# Your task is to create a function called generate_markdowns or generateMarkdowns. It will take
# three parameters:

# markdown - The type of markdown to return. It can be "link", "img" or "code".
# text - The text,message or code to display
# url_or_language or urlOrLanguage - gives either the url address to use or the name of the language
# for the codeblock.
# Examples
...
def generate_markdowns(markdown, text, url_or_language):
    if markdown in ["link", "img"]:
        generated_markdown = f"[{text}]({url_or_language})"

        if markdown == "img":
            generated_markdown = "!" + generated_markdown
    else:
        generated_markdown = f"```{url_or_language}\n{text}\n```"
